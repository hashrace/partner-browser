...
```json
{
  "data": {
    ...
    "currency": "USDT",
    "target_mode": "game",
    "allowed_parent_origins": ["https://www.example.com"],
...
| `data.allowed_parent_origins` | string[] | 否 | 允许嵌入游戏的父页 origin 列表，**决定 postMessage 通道是否开通**（见 §5.1）。每条形如 `https://www.example.com`：必须 `https://`、不带路径 / query / 末尾斜杠、host 小写；可用 `*` 通配**单级**子域（`https://*.example.com`），但不接受 `https://*.*`、`https://*example.com` 这类等于全网的写法。最多 16 条，单条 ≤ 300 字节 |
...
| `INVALID_PARAMS` | 400 | 字段非法：`external_player_id` 为空 / 过长、`allowed_parent_origins` 写法不合规、币种格式不对 | 按 `trace_id` 与 message 修正参数 |
...
- 只有 launch-session 请求里的 `allowed_parent_origins` **非空**时，游戏才会开通这条通道；否则不发送任何事件。
